...
sudo rm /etc/localtime && sudo ln -s /usr/share/zoneinfo/Europe/Kiev /etc/localtime
...
sudo mv files/prometheus-files/node_exporter.service /etc/systemd/system/node_exporter.service
sudo systemctl daemon-reload

sudo systemctl enable node_exporter
sudo systemctl start node_exporter

sudo mv files/prometheus-files/daemon.json /etc/docker/daemon.json
