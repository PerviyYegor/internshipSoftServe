...
resource "aws_key_pair" "docker_server_key" {
  key_name   = "terraform-docker"
...
resource "aws_instance" "apacheDocker" {
  ami                         = "ami-0aedf6b1cb669b4c7"
  instance_type               = "t2.micro"
  key_name                    = aws_key_pair.docker_server_key.key_name
  subnet_id                   = aws_subnet.main.id
...
resource "null_resource" "execute" {
  connection {
      ...
      host        = aws_instance.apacheDocker.public_ip
    }
  provisioner "file" {
    source      = "dockerInVMPart/"
    destination = "/home/centos"
  }
  provisioner "remote-exec" {
    inline = [
    "chmod +x startDockerDependencies.sh",
    "./startDockerDependencies.sh",
    "chmod +x MonitoringStart/prometheusAgentInstall.sh",
    "cd MonitoringStart",
    "./prometheusAgentInstall.sh"
    ]
  }
...
resource "local_file" "ip" {
  content  = aws_instance.apacheDocker.public_ip
  filename = "ip.txt"
}
