...
resource "aws_key_pair" "prometheus_server_key" {
  key_name   = "terraform-prometheus"
...
resource "aws_instance" "prometheusHost" {
  ami                         = "ami-0aedf6b1cb669b4c7"
  instance_type               = "t2.micro"
  key_name                    = aws_key_pair.prometheus_server_key.key_name
  subnet_id                   = aws_subnet.main.id
...
resource "null_resource" "execute-local" {
  provisioner "local-exec" {
    command = "./prometheusConfScript.sh"
  }
...
resource "null_resource" "execute-remote" {
  connection {
      ...
      host        = aws_instance.prometheusHost.public_ip
    }
  provisioner "file" {
    source      = "VMPart/"
...
resource "local_file" "ip" {
  content  = aws_instance.prometheusHost.public_ip
  filename = "ip.txt"
}
